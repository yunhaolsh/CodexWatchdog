...
- [x] 增加旧头像协议 TextMessage 编码及 0x10/0x11 应用层心跳；自动化验证回复、超时和重连，实机已观测连续心跳。
- [x] 恢复 USB `44:1B:F6:E5:62:28` 固件并接入 AVATAR；用户确认屏幕显示 Ready。不得操作另一块 `AC:27:6E:D2:FD:C8`，不能按 ACM 编号识别。
- [ ] 动态状态屏幕验收：真实只读任务已成功、设备保持连接，执行中与完成文字仍待用户确认。
...
- [x] 实现 WATCHDOG 常驻显示页：当前阶段、可滚动正文、最终回复和在线/离线标识；用户已确认完成文字常驻，滚动和离线效果待验收；已补充 common 中文字体。
- [ ] 增加真实权限内容和 Allow/Reject，接入鉴权协议。
- [x] 实现状态灯：idle 熄灭、running 红闪、waiting 黄闪、success 绿常亮、failed 红常亮；用户已确认成功绿灯，其余灯效待实机验收。
- [x] 实现 12 秒心跳超时显示 OFFLINE、文字变灰、灯熄灭，重连恢复（待实机验收）。
- [x] 实现完成/失败短提示音；重复完成快照不重复播放；增加试听按钮、增强音量和时长，听觉效果待实机验收。
