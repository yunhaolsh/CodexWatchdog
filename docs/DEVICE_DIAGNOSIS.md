...
用户确认完成绿灯正常、文字一直保留；实际显示“本完成”，未听到提示音。
检查字体源码确认精简 font_puhui_basic_20_4 不含 U+8F6E“轮”，完整字体包含。
WATCHDOG 已改为从 assets/index.json 的 text_font 加载 common 字体，
并在进入页面时日志报告是否取得真实 U+8F6E 字形，排除缺字占位符误报。

提示音增加右上角“试听”按钮，不需要调用 Codex 即可手动触发同一播放路径。
三音从每音 125ms 延长为 250ms，PCM 峰值从 1800 调整为 6000；
临时音量为 75%，播放后恢复原内存音量，并通过已有 HAL 接口保留持久设置。
增加前后 200ms 静音以稳定放大器并留出 DMA 排空时间。
日志区分开始、结束、扬声器忙以及任务创建失败；同时修正 seq=ld 日志格式。

独立副本编译通过，应用大小 3835712 字节，SHA256 为
`aef74d58c502286dd579b5830b9baaaad5fa66f160549072ca6fe5a762a09d1c`。
备份当前配置、核验 ota_0 后仅刷写应用分区，校验成功。复位日志确认
8MB PSRAM 和 Launcher 正常，观察期间无 panic。没有改动 PC 端代码。
新字体及声音仍需用户重新进入 WATCHDOG、观察“轮”字并点击“试听”确认；
本次尚无新页面打开后的字形检查或播放日志，不宣称已完成听觉验收。


## 提示音按用户要求调整至 100%

用户确认点击“试听”可以听到声音，但仍偏小，明确要求 100%。
已将试听与自动任务提示音共用的 notification_volume 从 75 调为 100；
播放后仍恢复原音量，音符、时长和 PCM 幅度保持上一版本设置。

固件编译通过；配置备份并核验 ota_0 后仅更新应用分区，校验成功。
应用 SHA256：5d885697e616ecc85197747ce5e458283771691adc46ceca78bce9ba36bdee0c。
复位日志确认 8MB PSRAM 与 Launcher 正常启动。100% 版本的实际音量
仍待用户重新打开 WATCHDOG 并点击“试听”确认。
