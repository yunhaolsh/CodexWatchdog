...
- 从 assets 的 index.json 加载 common 中文字体，避免精简内置字体缺少“轮”等字。
...
- 右上角“试听”按钮可直接验证扬声器，不需要运行任务。提示音为约 0.75 秒三音，
  前后各加约 0.2 秒静音用于输出稳定；临时使用 100% 音量，结束恢复原音量设置。
...
先检查“本轮完成”的“轮”字并点击右上角“试听”，再观察正文、滚动、红灯、绿灯
和自动提示音；完成后等待至少 30 秒，文字不应消失。
